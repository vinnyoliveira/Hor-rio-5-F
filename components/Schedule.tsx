...
import { SCHEDULE_DATA, DAYS_OF_WEEK_KEYS } from '../constants';
import type { ScheduleView, TimeSlot, DaySchedule } from '../types';
...
const Schedule: React.FC<ScheduleProps> = ({ view, activeFilters }) => {
  ...
  const filteredScheduleData = useMemo(() => {
    ...
    const dataCopy: TimeSlot[] = JSON.parse(JSON.stringify(SCHEDULE_DATA));

    dataCopy.forEach((slot, rowIndex) => {
      DAYS_OF_WEEK_KEYS.forEach(dayKey => {
        const daySchedule = slot[dayKey] as DaySchedule;
        if (Array.isArray(daySchedule)) {
          const originalClass = (SCHEDULE_DATA[rowIndex][dayKey] as any[])[0];
