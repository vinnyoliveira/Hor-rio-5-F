...
const FilterModal = ({ isOpen, onClose, subjects, activeFilters, onFilterChange, onClearFilters }) => {
  ...
  return React.createElement("div", {
    ...
    React.createElement("div", {
      ...
      React.createElement("header", { className: "flex items-center justify-between p-4 border-b border-gray-700" },
        React.createElement("h2", { className: "text-xl font-bold text-white" }, "Filtrar Matérias"),
      ...
      React.createElement("div", { className: "p-4 overflow-y-auto" },
        React.createElement("ul", { className: "grid grid-cols-1 sm:grid-cols-2 gap-2" },
          subjects.map(subject => (
            React.createElement("li", { key: subject.key },
              React.createElement("label", { className: `flex items-center gap-4 p-3 rounded-lg cursor-pointer transition-all duration-200 border-2 ${activeFilters.includes(subject.key) ? 'bg-indigo-600/30 border-indigo-500' : 'bg-gray-800/50 border-transparent hover:border-gray-600'}` },
                ...
                React.createElement("span", { className: "text-3xl" }, subject.icon),
                React.createElement("span", { className: "font-medium text-gray-200" }, subject.fullName)
              )
            )
          ))
        )
      ),
      React.createElement("footer", { className: "flex items-center justify-between p-4 mt-auto border-t border-gray-700" },
         React.createElement("button", {
